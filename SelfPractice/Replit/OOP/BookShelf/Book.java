...
public class Book {
     ...
     * capitalize("smith john") => "Smith John"
     * capitalize("") => ""
    ...
    public static Boolean isTitleWord(String word) {
        ...
        String[] coordConj = {"and", "but", "for", "nor", "or", "so", "yet"};
        String[] prepposit = {"at", "by", "in", "into", "near", "of", "on", "to", "than", "via"};
        String[][] words = {articles, coordConj, prepposit};
        Boolean result = true;
        for (String[] arcopr : words) {
            for (String each : arcopr) {
     ...
     * @param author : String
     *               Conditions:
     *               => if parameter author is value is empty,
     *               1) "Unknown" is assigned to instance variable author
    ...
    public void setTitle(String title) {
        ...
        if (title.isEmpty()) {
            ...
            result = capitalize(title);
        } else {
            String[] wordsArr = title.split(" ");
            for (int i = 0; i < wordsArr.length; i++) {
                if (i == 0) {
                    wordsArr[i] = wordsArr[i].substring(0, 1).toUpperCase() + wordsArr[i].substring(1).toLowerCase();
                } else if (isTitleWord(wordsArr[i])) {
                    wordsArr[i] = capitalize(wordsArr[i]);
                } else if (!isTitleWord(wordsArr[i])) {
